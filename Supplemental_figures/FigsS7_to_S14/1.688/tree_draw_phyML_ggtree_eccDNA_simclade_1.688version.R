...
infile <- "~/Desktop/raxml_satDNA/RAxML_bipartitions.dmau_1.688_Xchrom_uniqhet_dere_MAFFT.phy_automre.converted.done"
spp <- "dmau"
reptype <- "1.688"
...
mapping <- read.table(sprintf("~/Desktop/satDNA_mapping/4-mauE_1pt688.txt",spp,reptype),sep="\t",header=T)
...
#Assign RPM value to each variant in the annotations data frame (dd) based on mapping data:
for(i in 1:nrow(dd)) {
...
##############################################################  
  ...
  for(j in 1:nrow(mapping)) {
    ...
    mapcoord <- sub("U_","U.",mapcoord) #annoying formatting thing with the U contigs
    mapcoord <- sub("Y_","Y.",mapcoord) #ditto
    mapstart <- as.numeric(unlist(strsplit(mapcoord,"_"))[2])
