...
infile <- "~/Desktop/raxml_satDNA/RAxML_bipartitions.dmel_rsp_Xchrom_uniqhet_dere_MAFFT.phy_automre"
spp <- "dmel"
reptype <- "rsp"
...
mapping <- read.table(sprintf("~/Desktop/satDNA_mapping/dmel_eccDNA_vs_cel8.1_r6_combo_rsp.txt",spp,reptype),sep="\t",header=T)
...
#dd$cyto <- factor(dd$cyto, levels = sort(as.numeric(levels(dd$cyto))))
...
#Assign RPM values to each variant in the annotations data frame (dd) based on mapping data:
for(i in 1:nrow(dd)) {
  ...
  #Parsing the labels in data frames properly, a little cumbersome
  if(grepl("utg",label)) {
  ...
  for(j in 1:nrow(mapping)) {
    ...
    if(grepl("utg",mapcoord)) {
      ...
      mapstart <- as.numeric(unlist(strsplit(mapcoord,"_"))[6]) #parse it and save as variable = "start"
      mapend <- as.numeric(unlist(strsplit(mapcoord,"_"))[7])
    } else {
      mapstart <- as.numeric(unlist(strsplit(mapcoord,"_"))[3]) #parse it and save as variable = "start"
      mapend <- as.numeric(unlist(strsplit(mapcoord,"_"))[4]) 
    }
